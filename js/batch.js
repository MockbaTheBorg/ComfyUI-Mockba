        ...
        if (nodeData.name === "mbBatchInput" || nodeData.name === "mbBatchOutput") {
            ...
            nodeType.prototype.onNodeCreated = function() {
                ...
                setTimeout(() => {
                    ...
                    if (configWidget) {
                        // Completely hide the configuration widget to prevent size issues
                        configWidget.hidden = true;
                        configWidget.type = "hidden";
                        configWidget.computeSize = () => [0, 0];
                        configWidget.onDraw = () => {}; // Prevent drawing
                        
                        // Override the widget's layout contribution
                        ...
                        // Force node to recalculate size without the hidden widget
                        setTimeout(() => {
                            if (this.computeSize) {
            ...
            nodeType.prototype.showConfigurationPopup = function() {
                ...
                // Make the widget temporarily visible for value reading
                const configWidget = node.widgets?.find(w => w.name === configField);
                if (configWidget) {
                    const wasHidden = configWidget.hidden;
                    configWidget.hidden = false;
                    
                    const value = prompt(`How many ${configField} do you want? (2-16)`, "4");
                    
                    // Restore hidden state
                    configWidget.hidden = wasHidden;
            ...
            // Add method to expose hidden widgets for scanning (for popups, etc.)
            nodeType.prototype.getConfigWidget = function() {
                const isInput = nodeData.name === "mbBatchInput";
                const configField = isInput ? "inputs" : "outputs";
                return this.widgets?.find(w => w.name === configField);
            };
            
            // Override any widget scanning to include our hidden config widget
            nodeType.prototype.getScanableWidgets = function() {
                const configWidget = this.getConfigWidget();
                return configWidget ? [configWidget] : [];
            };
        }
    }
});
